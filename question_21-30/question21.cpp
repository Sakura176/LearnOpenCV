...
cv::Mat hist_normal(cv::Mat image, int a, int b)
{
    ...
    for (int j = 0; j < width; j++)
    {
        for (int i = 0; i < height; i++)
        {
            ...
            {
                temp = (float)image.at<cv::Vec3b>(i, j)[k];
                c = fmin(c, temp);
                d = fmax(d, temp);
            }
        }
    ...
    for (int j = 0; j < width; j++)
    {
        for (int i = 0; i < height; i++)
        {
            for (int k = 0; k < channel; k++)
            {
                tem = image.at<cv::Vec3b>(i, j)[k];
                if (tem < c)
                {
                    out.at<cv::Vec3b>(i, j)[k] = (uchar)a;
                }
                else if (tem < d)
                {
                    out.at<cv::Vec3b>(i, j)[k] = (uchar)(b - a) / (d - c) * (tem - c) + a;
                }
                else
                {
                    out.at<cv::Vec3b>(i, j)[k] = (uchar)b;
                }
            }
        }
    }
