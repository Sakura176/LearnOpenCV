 ...
 */
cv::Mat ImgRotate(const cv::Mat img, const int rotate_angle);
 ...
 */
cv::Mat ImgInPaint(const cv::Mat img, const cv::Mat mask);
...
int main(int argc, const char *argv[]) {
	...
	cv::Mat rotate_img = ImgRotate(img, 5);
	...
	cv::Mat inpaint = ImgInPaint(rotate_img, mask);
...
cv::Mat ImgRotate(const cv::Mat img, const int rotate_angle) {
	...
	// 创建旋转矩阵
	cv::Mat rotate_matrix = cv::getRotationMatrix2D(center, rotate_angle, 1.0);

	// 图片旋转
	cv::Mat rotate_img;
	cv::warpAffine(img, rotate_img, rotate_matrix, img.size());
...
cv::Mat ImgInPaint(const cv::Mat img, const cv::Mat mask) {
	...
	cv::Mat inpaint_img;
	cv::inpaint(img, mask, inpaint_img, 20, cv::INPAINT_NS);
