...
/***
 * @description: Emboss滤波器，使物体轮廓更加清晰
 * @param image 输入图片
 * @param ksize 滤波器的大小
 * @return Mat
 * | -2 -1  0 |  
 ...
 * |  0  1  2 |
 */
cv::Mat Emboss_filter(cv::Mat image, cv::Size ksize);

int main(int argc, char const *argv[])
{
    cv::Mat image = imread(argv[1], cv::IMREAD_GRAYSCALE);
    cv::Mat out = Emboss_filter(image, cv::Size(3, 3));
    imshow("image", image);
    imshow("out", out);
    cv::waitKey(0);
