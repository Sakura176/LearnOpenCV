...
cv::Mat differential_filter(cv::Mat image, cv::Size ksize, bool horzontal=true);

int main(int argc, char const *argv[])
{
    cv::Mat image = imread(argv[1], cv::IMREAD_GRAYSCALE);
    cv::Mat out = differential_filter(image, cv::Size(3, 3), false);
    imshow("image", image);
    imshow("out", out);
    cv::waitKey(0);
...
cv::Mat differential_filter(cv::Mat image, cv::Size ksize, bool horzontal)
{
